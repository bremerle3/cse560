...
public class Cache {
	...
	nIndexBits = log2(capacity/(blockSize*associativity));
	...
	// fix this!
	long mask = (long)(Math.pow(2,nTagBits)-1)<<(nIndexBits + nOffsetBits);
	return (addr & mask)>>(nIndexBits + nOffsetBits);
    }
    
    public int getIndex(long addr) {
	
	// fix this!
	long mask = (long)(Math.pow(2,nIndexBits)-1)<<(nOffsetBits);
	//return (addr&mask)>>(nOffsetBits);
	return (int)((addr & mask)>>(nOffsetBits));   //Why does this return int???
    }
    
    public long getBlockAddress(long addr) {
	
	// fix this!
	long mask = (long)(Math.pow(2,nTagBits+nIndexBits)-1)<<(nOffsetBits);
	return (addr & mask);
    }
	...
	for (int i = 0; i < associativity; i++){
	    if(cacheTags[getIndex(PC)][i] == getTag(PC)){  //cache hit
	...
	result = false;
	int accessedWay = LRU_way[getIndex(PC)];
	cacheTags[getIndex(PC)][accessedWay] = getTag(PC);
	updateLRU(getIndex(PC), accessedWay);	  
	if(dirtyBits[getIndex(PC)][accessedWay]){
	    dirtyEvic_f = true;
	    dirtyBits[getIndex(PC)][accessedWay] = false;
	}
	if(!isLoad){
	...
	/*	System.out.format("[S0: {W0: %s, %s} {W1: %s, %s} LRU=%d] [S1: {W0: %s, %s} {W1: %s, %s} LRU=%d] |\t ", Long.toHexString(cacheTags[0][0]), dirtyBits[0][0],Long.toHexString(cacheTags[0][1]), dirtyBits[0][1], LRU_way[0], Long.toHexString(cacheTags[1][0]),dirtyBits[1][0], Long.toHexString(cacheTags[1][1]),dirtyBits[0][0], LRU_way[1]);		*/
		}
